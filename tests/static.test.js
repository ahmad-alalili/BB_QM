...
test('the public version is 2.5 while internal package versions stay synchronized', () => {
  const html = read('index.html');
  ...
  assert.equal(core.DISPLAY_VERSION, '2.5');
  assert.match(html, /id="app-version">2\.5<\/span>/);
  assert.match(html, /تنزيل دليل المستخدم 2\.5 \(PDF\)/);
  const guidePath = attributeValues(html.match(/<a class="guide-download"[^>]*>/)[0], 'href')[0];
  assert.equal(guidePath, 'docs/دليل-استخدام-منشئ-أسئلة-بلاكبورد-2.5.pdf');
  assert.equal(fs.readFileSync(path.join(root, guidePath)).subarray(0, 5).toString(), '%PDF-');
